...
**Framing note (2026-08-31).** The product pivoted from succession intelligence
to the dossier — memory with due process, the same engine pointed at a person's
own guidance instead of an organization's records. The pivot and its reasoning
are recorded in `docs/pivot/DECISION-dossier.md` and as PRD v2.0
(`docs/PRD.md` §6). The compliance substance below is unchanged by the pivot:
the same log, fold, detection, approval and visibility machinery carries both
framings, and `src/baraza/successor/` is now `src/baraza/dossier/` (mechanical
rename; the test suite is the net). Rows below name the current paths.
...
| **At least one Google Cloud infrastructure service** (recovered) | Cloud Run Jobs for the ingestion Job and the nightly `baraza-reconcile` Job; Cloud Run services for the private session surface, the public dossier surface, and the `baraza-trigger` OIDC hop that lets Scheduler start the reconcile Job; Firestore as the append-only claim-event log, session store and entity table; Cloud Scheduler as the nightly trigger. | BAR-021, BAR-410, BAR-411† | `src/baraza/fold/store.py`, `src/baraza/reconcile/job.py`; deployment state in the README's status table |
| **Google models perform the work** (in-tree inference) | Gemini in two roles: a reasoning role for contradiction adjudication, agenda synthesis, the divergence turn and dossier synthesis; a fast role for extraction, alias proposals and session turns. **The Gemini pins were live-verified 2026-08-31 against project `baraza-2026`** — the resolution `make verify-models` performs — after the original pins turned out to name a model absent from the Vertex catalog, the exact plausible-literal defect the pin module's docstring warns about; the verified IDs and location are recorded once, in the README's Google Cloud table, and the pins live only in `src/baraza/schema/models.py`. Gemma remains **pinned and wired for** the ingestion relevance pre-filter but has never run: unattended ingestion uses the `stub`, and the `gemma` branch calls the ordinary `generate_content` path while the pin declares `surface="vertex-endpoint"` — nothing reads `GemmaFilter.endpoint`, so a live attempt would fail open on every chunk. That failure is legible rather than silent (`FilterReport.failed_open`; a degraded pass prints `DEGRADED` and writes `not yet measured`), and the BAR-303 bonus is not claimed. | BAR-303, BAR-320, BAR-330 | `make compliance` fails on a model-ID literal anywhere else; `make verify-models` resolves every pin against live Vertex |
...
| **Architecture diagram** (in-tree inference) | `docs/architecture.md` (Mermaid, renders on GitHub) and `docs/architecture.svg` (self-contained, legible in light and dark). Both show the sources, the model roles, the append-only log, the fold, on-write detection aimed at the user, the ledger, agenda and doctrine, the partner session with the divergence card, the model-free promotion-isolated approval gate, the visibility boundary, the public dossier surface, and the Scheduler → trigger → Job chain. Neither prints a model ID; the live-verified IDs appear once, in the README's Google Cloud table. | BAR-505† | `docs/architecture.md`, `docs/architecture.svg` |
...
| **Publicly reachable hosted instance** (in-tree inference) | The deployed public dossier surface (`src/baraza/dossier/service.py`) reads as `Audience.PUBLIC`, so a logged-out judge sees only explicitly public committed claims. It exposes read-only `/`, `/ledger`, and `/agenda` views, all rendered through the same audience boundary. The original Scheduler → Jobs-API path 403'd; the root cause and the `baraza-trigger` OIDC-hop fix are recorded in `docs/deploy-postmortem.md`, and `scheduler_nightly_runs_completed` in `docs/metrics.json` remains `not yet measured` — no scheduled-run count is claimed until it is measured. | BAR-410, BAR-411 | `docs/deploy-postmortem.md`; `tests/unit/test_public_surfaces.py` |
