...
| **Publicly reachable hosted instance** (in-tree inference) | The deployed public dossier surface (`src/baraza/dossier/service.py`) reads as `Audience.PUBLIC`, so a logged-out judge sees only explicitly public committed claims. It exposes read-only `/`, `/ledger`, and `/agenda` views, all rendered through the same audience boundary. The original Scheduler → Jobs-API path 403'd; the root cause and the `baraza-trigger` OIDC-hop fix are recorded in `STOPPED-DEPLOY.md`, and `scheduler_nightly_runs_completed` in `docs/metrics.json` remains `not yet measured` — no scheduled-run count is claimed until it is measured. | BAR-410, BAR-411 | `STOPPED-DEPLOY.md`; `tests/unit/test_public_surfaces.py` |
