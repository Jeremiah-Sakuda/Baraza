...
def stage_replay_interview(
    ...
    *,
    ...
    console.detail(persona.describe())
    console.note(f"      {persona.description}")
    if not paced:
        console.warn(
    ...
    harness = ReplayHarness(
        ...
        emit=console.write,
        on_answer=lambda text: (console.line(), console.officer(text)),
    )
    result = harness.run()
    ...
    console.note(
        "      generated by this run; never hand-edited. The published metric "
        "comes from"
    )
    console.note("      make adaptation-metric, which imports nothing from this package.")
    return result
...
def stage_live_interview(
    ...
    *,
    ...
    discipline with the replay path; only the source of the answers differs.
    That is deliberate — a replay harness that exercised a different code path
    would be evidence about the harness.
    """
    console.stage("interview", "interactive — type an answer, blank line to end")

    interviewer = Interviewer(client, state, audience=audience)
    session_store = SessionStore(store)
    adaptation = AdaptationState(persona_id=persona_id)

    now = to_epoch_millis(time.time(), field="interview.opened_at")
    session = session_store.open(persona_id=persona_id, opened_at=now)
    items = {item.item_id: item for item in agenda.items}

    plan = interviewer.opening_turn(agenda, session)
    current_item = items[plan.agenda_item_id]
    last_instant = now
    ...
    while True:
        ...
        agent_turn = _append_turn(
            session_store,
            session,
            role=TurnRole.AGENT,
            kind=plan.kind,
            text=plan.text,
            occurred_at=last_instant,
        ...
        change = adaptation.observe(answer, turn_id=officer_turn.turn_id)
        if change:
            console.note(f"      [adaptation] {change}")
    ...
    session_store.close(
        session, closed_at=max(last_instant + 1, to_epoch_millis(time.time()))
    )
    console.line()
    console.detail(f"turns recorded      {len(session.turns)}")
    return session


def _append_turn(session_store: SessionStore, session: Session, **kwargs: Any):
    from baraza.schema.session import Turn

    turn = Turn.create(session_id=session.session_id, index=session.next_index, **kwargs)
    session.turns.append(turn)
    # Externalized before the next turn is solicited. BAR-334's property depends
    # on this ordering and on nothing else.
...
def stage_approval(
    ...
    *,
    ...
    # it silently, and the successor would see an empty handover with nothing
...
def cmd_demo(args: argparse.Namespace, console: Console) -> int:
    console.title(
        ...
        "ingest → agenda → replay interview → approval → successor query",
    )
    client = build_client(
        offline=args.offline, cassette_dir=args.cassettes, delay_ms=args.delay_ms
    )
...
def _add_interview_options(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--persona",
        default="terse",
        help=f"replay persona; available: {', '.join(available_personas()) or '(none)'}",
    )
    parser.add_argument(
        "--speed",
        type=float,
        default=4.0,
        help="acceleration applied to the persona's declared typing pace",
    )
    parser.add_argument(
        "--no-timer",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="agenda items to cover, so both personas are scored over comparable "
        "ground",
    )
    parser.add_argument(
        "--transcripts",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="baraza",
        description="Succession intelligence — demo entrypoints for the Makefile.",
    )
    sub = parser.add_subparsers(dest="command", required=True)

    demo = sub.add_parser(
        "demo", help="ingest → agenda → replay interview → approval → successor query"
    )
    _add_common(demo)
    ...
    interview.add_argument(
        "--replay",
        action="store_true",
        help="feed canned answers from a persona fixture on a timer",
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        console.fail(f"persona fixture rejected:\n{exc}")
        return EXIT_PRECONDITION
